...
export const StaggeredMenu = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(plusH, { transformOrigin: '50% 50%', rotate: 0, top: '20%' });
      gsap.set(plusV, { transformOrigin: '50% 50%', rotate: 0, top: '80%' });
      if (middleLineRef.current) gsap.set(middleLineRef.current, { opacity: 1, top: '50%', rotate: 0, transformOrigin: '50% 50%' });
  ...
  const animateIcon = useCallback((opening: boolean) => {
    ...
    if (opening) {
      ...
      spinTweenRef.current = gsap
        ...
        .to(h, { top: '50%', rotate: 45, duration: 0.5 }, 0)
        .to(v, { top: '50%', rotate: -45, duration: 0.5 }, 0)
        .to(m, { opacity: 0, duration: 0.2 }, 0);
    } else {
      spinTweenRef.current = gsap
        .timeline({ defaults: { ease: 'power3.inOut' } })
        .to(h, { top: '20%', rotate: 0, duration: 0.35 }, 0)
        .to(v, { top: '80%', rotate: 0, duration: 0.35 }, 0)
        .to(m, { opacity: 1, duration: 0.35 }, 0)
  ...
  const animateColor = useCallback(
    (opening: boolean) => {
      ...
      const baseColor = !isDesktop ? '#FFFFFF' : menuButtonColor;
    ...
    [openMenuButtonColor, menuButtonColor, changeMenuColorOnOpen, isDesktop]
  );
  ...
  React.useEffect(() => {
    if (toggleBtnRef.current) {
      const baseColor = !isDesktop ? '#FFFFFF' : menuButtonColor;
      if (changeMenuColorOnOpen) {
        const targetColor = openRef.current ? openMenuButtonColor : baseColor;
        gsap.set(toggleBtnRef.current, { color: targetColor });
      } else {
        gsap.set(toggleBtnRef.current, { color: baseColor });
      }
    }
  }, [changeMenuColorOnOpen, menuButtonColor, openMenuButtonColor, isDesktop]);
        ...
        >
          <div className="sm-logo flex items-center select-none pointer-events-auto" aria-label="Logo">
            <Link to="/">
              <img
                src={logoUrl}
                alt="Logo"
                className={`sm-logo-img block h-6 md:h-10 w-auto object-contain transition-all duration-300 hover:opacity-85 ${!isDesktop ? 'brightness-0 invert' : ''}`}
                draggable={false}
              />
            </Link>
          </div>
          ...
          <div className="flex items-center gap-10">
            {navLinks && navLinks.length > 0 && (
              <div className={`hidden md:flex items-center gap-10 pointer-events-auto transition-all duration-300 ${scrolled ? 'opacity-0 pointer-events-none translate-y-[-10px]' : 'opacity-100 translate-y-0'}`}>
